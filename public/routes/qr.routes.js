...
const QrCode = Router();
QrCode.put("/add", upload.single("qrCodeImage"), asyncHandler(async (req, res, next) => {
    const walletaddress = req.body.walletaddress;
    const image = req.file;
    // ✅ Validate inputs
    if (!image && !walletaddress) {
        return next(new ErrorResponse("Either walletaddress or qrCodeImage is required", statusCode.Bad_Request));
    }
    if (image) {
    ...
    if (walletaddress && walletaddress.length < 26) { // Example: Basic wallet address length check (adjust as needed)
        return next(new ErrorResponse("Wallet address must be at least 26 characters", statusCode.Bad_Request));
    }
    try {
        ...
        if (qrCode?.qrCodeUrl) {
            const existingImage = qrCode.qrCodeUrl;
        ...
        if (qrCode) {
            ...
            if (walletaddress)
                updateData.wallentaddress = walletaddress;
            if (cloudinaryResult)
                updateData.qrCodeUrl = cloudinaryResult;
...
QrCode.get("/get", asyncHandler(async (req, res, next) => {
    try {
